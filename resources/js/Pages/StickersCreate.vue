<template>
  <Head :title="`${data.title} Category Sticker Page | `" />
  <div class="max-w-2xl mx-auto mt-10 bg-gradient-to-br from-gray-900 to-gray-600 rounded-2xl shadow-xl p-8 text-gray-200">
    <h2 class="text-2xl font-bold text-white text-center mb-6">Create Sticker of {{ data.title }}</h2>

    <form @submit.prevent="submit">
      <input type="hidden" v-model="form.category_id" />

      <div v-for="(item, index) in editableEmojis" :key="index" class="relative mb-4">
        <input
          type="text"
          v-model="item.emoji"
          class="w-full pr-10 px-4 py-2 rounded-lg border border-gray-700 bg-gray-800 text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
        ...
        <button
          type="button"
          @click="activeEmojiIndex = activeEmojiIndex === index ? null : index"
      ...
      <div class="mb-4">
        <label class="block text-gray-400 font-semibold mb-2">Source Url</label>
        <input
          type="url"
          v-model="form.link"
          pattern="https?://.+"
          class="w-full px-4 py-2 border border-gray-700 bg-gray-800 text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
      ...
      <div class="mb-4 hidden">
        <label class="block text-gray-400 font-semibold mb-2">Type</label>
        <select
          v-model="form.type"
          class="w-full px-4 py-2 border border-gray-700 bg-gray-800 text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
        >
      ...
      <div class="mb-4">
        <label class="block text-gray-400 font-semibold mb-2">Status</label>
        <select
          v-model="form.status"
          class="w-full px-4 py-2 border border-gray-700 bg-gray-800 text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
        >
      ...
      <div class="mb-4">
        <label class="block text-gray-400 font-semibold mb-2">Sticker</label>
        <div class="bg-gray-800 flex items-center justify-center h-36 rounded-lg">
          <img
            :src="previewImage || `/images/default-image.webp`"
            alt="Sticker preview"
...
async function submit() {
    ...
    form.post(route('stickers.create'), {
